...
public class JavaMethodFluent<T extends JavaMethodFluent<T>> extends AttributeSupportFluent<T> implements Fluent<T>{
     ...
     Set<VisitableBuilder<JavaType, ?>> annotations = new LinkedHashSet();     Set<Modifier> modifiers = new LinkedHashSet();     Set<VisitableBuilder<JavaType, ?>> typeParameters = new LinkedHashSet();     String name;     VisitableBuilder<JavaType, ?> returnType;     List<VisitableBuilder<JavaProperty, ?>> arguments = new ArrayList();     Set<VisitableBuilder<JavaType, ?>> exceptions = new LinkedHashSet();
    ...
    for (Modifier item : items) {this.modifiers.add(item);} return (T)this;
    }
    public T removeFromModifiers( Modifier ...items){
    for (Modifier item : items) {this.modifiers.remove(item);} return (T)this;
    }
    public Set<Modifier> getModifiers(){
    return this.modifiers;
    }
    public T withModifiers( Set<Modifier> modifiers){
    this.modifiers.clear();if (modifiers != null) {for (Modifier item : modifiers){this.addToModifiers(item);}} return (T) this;
    }
    public T withModifiers( Modifier ...modifiers){
    this.modifiers.clear(); if (modifiers != null) {for (Modifier item :modifiers){ this.addToModifiers(item);}} return (T) this;
    }
    public T addToTypeParameters( JavaType ...items){
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.add(builder);this.typeParameters.add(builder);} return (T)this;
    }
    public T removeFromTypeParameters( JavaType ...items){
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.remove(builder);this.typeParameters.remove(builder);} return (T)this;
    }
    public Set<JavaType> getTypeParameters(){
    return build(typeParameters);
    }
    public T withTypeParameters( Set<JavaType> typeParameters){
    this.typeParameters.clear();if (typeParameters != null) {for (JavaType item : typeParameters){this.addToTypeParameters(item);}} return (T) this;
    }
    ...
    if (returnType!=null){ this.returnType= new JavaTypeBuilder(returnType); _visitables.add(this.returnType);} return (T) this;
    }
    ...
    this.arguments.clear(); if (arguments != null) {for (JavaProperty item :arguments){ this.addToArguments(item);}} return (T) this;
    }
...
return result.toArray(new JavaProperty[result.size()]);
    ...
    for (JavaProperty item : items) {JavaPropertyBuilder builder = new JavaPropertyBuilder(item);_visitables.add(builder);this.arguments.add(builder);} return (T)this;
    }
    public T removeFromArguments( JavaProperty ...items){
    for (JavaProperty item : items) {JavaPropertyBuilder builder = new JavaPropertyBuilder(item);_visitables.remove(builder);this.arguments.remove(builder);} return (T)this;
    }
    ...
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.add(builder);this.exceptions.add(builder);} return (T)this;
    }
    public T removeFromExceptions( JavaType ...items){
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.remove(builder);this.exceptions.remove(builder);} return (T)this;
    }
    public Set<JavaType> getExceptions(){
    return build(exceptions);
    }
    public T withExceptions( Set<JavaType> exceptions){
    this.exceptions.clear();if (exceptions != null) {for (JavaType item : exceptions){this.addToExceptions(item);}} return (T) this;
    }
