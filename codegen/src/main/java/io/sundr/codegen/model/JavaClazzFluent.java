...
public class JavaClazzFluent<T extends JavaClazzFluent<T>> extends AttributeSupportFluent<T> implements Fluent<T>{
     ...
     VisitableBuilder<JavaType, ?> type;     Set<VisitableBuilder<JavaType, ?>> annotations = new LinkedHashSet();     Set<VisitableBuilder<JavaMethod, ?>> methods = new LinkedHashSet();     Set<VisitableBuilder<JavaMethod, ?>> constructors = new LinkedHashSet();     Set<VisitableBuilder<JavaProperty, ?>> fields = new LinkedHashSet();     Set<VisitableBuilder<JavaType, ?>> imports = new LinkedHashSet();     Set<VisitableBuilder<JavaClazz, ?>> nested = new LinkedHashSet();

    public JavaType getType(){
    return this.type!=null?this.type.build():null;
    }
    public T withType( JavaType type){
    if (type!=null){ this.type= new JavaTypeBuilder(type); _visitables.add(this.type);} return (T) this;
    }
    ...
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.add(builder);this.annotations.add(builder);} return (T)this;
    }
    public T removeFromAnnotations( JavaType ...items){
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.remove(builder);this.annotations.remove(builder);} return (T)this;
    }
    public Set<JavaType> getAnnotations(){
    return build(annotations);
    }
    public T withAnnotations( Set<JavaType> annotations){
    this.annotations.clear();if (annotations != null) {for (JavaType item : annotations){this.addToAnnotations(item);}} return (T) this;
    }
    ...
    for (JavaMethod item : items) {JavaMethodBuilder builder = new JavaMethodBuilder(item);_visitables.add(builder);this.methods.add(builder);} return (T)this;
    }
    public T removeFromMethods( JavaMethod ...items){
    for (JavaMethod item : items) {JavaMethodBuilder builder = new JavaMethodBuilder(item);_visitables.remove(builder);this.methods.remove(builder);} return (T)this;
    }
    public Set<JavaMethod> getMethods(){
    return build(methods);
    }
    public T withMethods( Set<JavaMethod> methods){
    this.methods.clear();if (methods != null) {for (JavaMethod item : methods){this.addToMethods(item);}} return (T) this;
    }
    ...
    for (JavaMethod item : items) {JavaMethodBuilder builder = new JavaMethodBuilder(item);_visitables.add(builder);this.constructors.add(builder);} return (T)this;
    }
    public T removeFromConstructors( JavaMethod ...items){
    for (JavaMethod item : items) {JavaMethodBuilder builder = new JavaMethodBuilder(item);_visitables.remove(builder);this.constructors.remove(builder);} return (T)this;
    }
    public Set<JavaMethod> getConstructors(){
    return build(constructors);
    }
    public T withConstructors( Set<JavaMethod> constructors){
    this.constructors.clear();if (constructors != null) {for (JavaMethod item : constructors){this.addToConstructors(item);}} return (T) this;
    }
    ...
    for (JavaProperty item : items) {JavaPropertyBuilder builder = new JavaPropertyBuilder(item);_visitables.add(builder);this.fields.add(builder);} return (T)this;
    }
    public T removeFromFields( JavaProperty ...items){
    for (JavaProperty item : items) {JavaPropertyBuilder builder = new JavaPropertyBuilder(item);_visitables.remove(builder);this.fields.remove(builder);} return (T)this;
    }
    public Set<JavaProperty> getFields(){
    return build(fields);
    }
    public T withFields( Set<JavaProperty> fields){
    this.fields.clear();if (fields != null) {for (JavaProperty item : fields){this.addToFields(item);}} return (T) this;
    }
    ...
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.add(builder);this.imports.add(builder);} return (T)this;
    }
    public T removeFromImports( JavaType ...items){
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.remove(builder);this.imports.remove(builder);} return (T)this;
    }
    public Set<JavaType> getImports(){
    return build(imports);
    }
    public T withImports( Set<JavaType> imports){
    this.imports.clear();if (imports != null) {for (JavaType item : imports){this.addToImports(item);}} return (T) this;
    }
    ...
    for (JavaClazz item : items) {JavaClazzBuilder builder = new JavaClazzBuilder(item);_visitables.add(builder);this.nested.add(builder);} return (T)this;
    }
    public T removeFromNested( JavaClazz ...items){
    for (JavaClazz item : items) {JavaClazzBuilder builder = new JavaClazzBuilder(item);_visitables.remove(builder);this.nested.remove(builder);} return (T)this;
    }
    public Set<JavaClazz> getNested(){
    return build(nested);
    }
    public T withNested( Set<JavaClazz> nested){
    this.nested.clear();if (nested != null) {for (JavaClazz item : nested){this.addToNested(item);}} return (T) this;
    }
