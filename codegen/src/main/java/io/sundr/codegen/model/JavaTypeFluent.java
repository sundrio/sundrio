...
public class JavaTypeFluent<T extends JavaTypeFluent<T>> extends AttributeSupportFluent<T> implements Fluent<T>{
     ...
     JavaKind kind;     String packageName;     String className;     boolean array;     boolean collection;     boolean concrete;     VisitableBuilder<JavaType, ?> defaultImplementation;     VisitableBuilder<JavaType, ?> superClass;     Set<VisitableBuilder<JavaType, ?>> interfaces = new LinkedHashSet();     List<VisitableBuilder<JavaType, ?>> genericTypes = new ArrayList();
    ...
    this.packageName=packageName; return (T) this;
    }
    public String getClassName(){
    return this.className;
    }
    public T withClassName( String className){
    this.className=className; return (T) this;
    }
    ...
    this.collection=collection; return (T) this;
    }
    public boolean isConcrete(){
    return this.concrete;
    }
    public T withConcrete( boolean concrete){
    this.concrete=concrete; return (T) this;
    }
    ...
    if (defaultImplementation!=null){ this.defaultImplementation= new JavaTypeBuilder(defaultImplementation); _visitables.add(this.defaultImplementation);} return (T) this;
    }
    ...
    if (superClass!=null){ this.superClass= new JavaTypeBuilder(superClass); _visitables.add(this.superClass);} return (T) this;
    }
    ...
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.add(builder);this.interfaces.add(builder);} return (T)this;
    }
    public T removeFromInterfaces( JavaType ...items){
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.remove(builder);this.interfaces.remove(builder);} return (T)this;
    }
    public Set<JavaType> getInterfaces(){
    return build(interfaces);
    }
    public T withInterfaces( Set<JavaType> interfaces){
    this.interfaces.clear();if (interfaces != null) {for (JavaType item : interfaces){this.addToInterfaces(item);}} return (T) this;
    }
    ...
    this.genericTypes.clear(); if (genericTypes != null) {for (JavaType item :genericTypes){ this.addToGenericTypes(item);}} return (T) this;
    }
...
return result.toArray(new JavaType[result.size()]);
    ...
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.add(builder);this.genericTypes.add(builder);} return (T)this;
    }
    public T removeFromGenericTypes( JavaType ...items){
    for (JavaType item : items) {JavaTypeBuilder builder = new JavaTypeBuilder(item);_visitables.remove(builder);this.genericTypes.remove(builder);} return (T)this;
    }
