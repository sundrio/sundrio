...
public class EagerTest {
    ...
    public void shouldRespectWithNull() {
        Eager item = new EagerBuilder().withList((List<Thing>)null).build();
        assertNull(item.getList());
    }

    @Test
    public void shouldRespectWithList() {
        Eager item = new EagerBuilder().withList(new ArrayList<Thing>()).build();
        assertNotNull(item.getList());
        assertEquals(0, item.getList().size());
    }
    ...
    public void shouldRespecAddToList() {
        Eager item = new EagerBuilder().addToList(new Thing(1, "foo")).build();
        assertNotNull(item.getList());
