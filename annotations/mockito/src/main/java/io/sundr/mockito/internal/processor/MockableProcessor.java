...
import io.sundr.mockito.internal.MockClasses;
...
public class MockableProcessor extends AbstractCodeGeneratingProcessor {
      ...
      for (Element element : roundEnv.getElementsAnnotatedWith(annotation)) {
        ...
        if (mockables != null) {
          ...
          for (TypeElement type : typesOf(mockables)) {
            MockTarget target = new MockTarget(adapt(type), mockables.prefix(), mockables.suffix(),
                mockables.verification());
            targets.add(target);
            warnOnTargetIssues(target);
            generate(MockClasses.mockClass(target));
          }
  ...
  private void generateMockClass(TypeDef definition, String prefix, String suffix, boolean verification) {
  ...
  private void generateAggregator(Element marker, String aggregator, List<MockTarget> targets) {
    ...
    generate(MockClasses.aggregatorClass(packageName, aggregator, targets));
  }
  ...
  private void warnOnTargetIssues(MockTarget target) {
    ...
    for (java.util.Map.Entry<String, String> skipped : target.getSkippedMethods().entrySet()) {
      processingEnv.getMessager().printMessage(Diagnostic.Kind.WARNING,
  ...
  private Set<TypeElement> typesOf(Mockables mockables) {
    Set<TypeElement> types = new java.util.LinkedHashSet<>();
    try {
      mockables.value();
