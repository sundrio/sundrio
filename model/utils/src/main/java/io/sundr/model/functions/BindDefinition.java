...
public class BindDefinition implements Function<ClassRef, TypeDef> {
  ...
  public TypeDef apply(ClassRef t) {
    ...
    return new TypeDefBuilder(definition)
        ...
        .accept(mapClassRefArguments(mappings), mapGenericFields(mappings), mapGenericReturnTypes(mappings))
        .build();
  }

  /**
   * Map generic fields to known {@link TypeRef} based on the specified mappings.
   * Example: Given a field {@code T size} and a map containing {@code T -> Integer} the final
   * field will be: {@code Integer type}.
   *
   * @param mappings A map that maps class arguments names to types.
   * @return a visitors that performs the actual mapping.
   */
  private static Visitor<FieldBuilder> mapGenericFields(Map<String, TypeRef> mappings) {
    return new Visitor<FieldBuilder>() {
      @Override
      public void visit(FieldBuilder property) {
