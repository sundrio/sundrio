...
public class ApplyTypeParamMappingToField implements Visitor<FieldFluent<?>> {

  private final Map<String, TypeRef> mappings;
  private final Optional<AttributeKey<TypeParamRef>> attributeKey;

  public ApplyTypeParamMappingToField(String name, TypeRef typeRef) {
    this(Maps.create(name, typeRef));
  }

  public ApplyTypeParamMappingToField(Map<String, TypeRef> mappings) {
  ...
  @Override
  public void visit(FieldFluent<?> field) {
    TypeRef typeRef = field.buildTypeRef();
    if (typeRef instanceof TypeParamRef) {
      ...
      if (paramRef != null) {
        field.withTypeRef(paramRef);
        attributeKey.ifPresent(k -> field.addToAttributes(k, typeParamRef));
      }
      ...
      if (classRef.getArguments().stream().anyMatch(a -> a instanceof TypeParamRef)) {
        ...
        field.withTypeRef(new ClassRefBuilder(classRef).withArguments(mappedArguments).build());
        attributeKey.ifPresent(k -> field.addToAttributes(k, classRef));
      }
    }
  }
}