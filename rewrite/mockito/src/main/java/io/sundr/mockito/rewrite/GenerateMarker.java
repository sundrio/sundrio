...
/**
 * Scans the module's test sources for every type used as a mock (see {@link MarkerPackages}),
 * collects the distinct set, computes their least-common-denominator package, and generates
 * {@code <lcd-package>/MocksConfig.java} annotated with {@code @Mockables({...})}. If a
 * {@code @Mockables}-annotated marker already exists (under any name) the value set is merged into
 * it rather than duplicated, and no new marker is generated.
 */
public class GenerateMarker extends ScanningRecipe<GenerateMarker.Accumulator> {
  ...
  static String markerSource(String pkg, Collection<String> types) {
    ...
    for (String fqn : types) {
      shortNames.add(MarkerPackages.simpleName(fqn) + ".class");
      if (!MarkerPackages.packageOf(fqn).equals(pkg)) {
